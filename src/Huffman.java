...
/* Huffman codificando , decodificando */
...
public class Huffman {
    ...
    private static int consoleMenu(Scanner scanner) {
        ...
        System.out.println("\n---- Menu ----\n" +
                ...
                "-- [2] codificar um texto\n" +
                "-- [3] decodificar um texto");
        decision = Integer.parseInt(scanner.nextLine());
        if (readFromFile)
            System.out.println("Decisão: " + decision + "\n---- Fim do menu ----\n");
        return decision;
    }
    ...
    private static void handleDecodingNewText(Scanner scanner) {
        System.out.println("Digite o texto para decodificar:");
        encoded = scanner.nextLine();
        System.out.println("Texto para decodificar: " + encoded);
        decodeText();
    }

    private static boolean handleEncodingNewText(Scanner scanner) {
        System.out.println("Digite o texto para codificar:");
        text = scanner.nextLine();
        System.out.println("Texto para Codificar: " + text);
    ...
    private static boolean handleNewText(Scanner scanner) {
        ...
        if (newTextBasedOnOldOne && (oldTextLength != 0 && !IsSameCharacterSet())) {
            ...
            printCodes();
            System.out.println("-- Codificação / Decodificação --");
            encodeText();
            decodeText();
            return false;
